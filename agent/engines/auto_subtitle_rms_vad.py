...
def _apply_zero_cross_refine_to_cues(
    ...
    """Spoken word start/end — RMS 스냅 직후 zero-crossing refine."""
    if samples.size < 8 or not cues:
        return cues
    ...
    for cue in cues:
        ...
        for w in raw_words:
            ...
            if _is_silence_token(w) or _is_pause_or_filler_token(w):
                ...
                refined.append({**w, "start": ws, "end": we})
                prev_global_end = max(prev_global_end, we)
                continue

            ws0 = float(w.get("start", 0))
            we0 = float(w.get("end", 0))
            if not (we0 > ws0 + 1e-6):
            ...
            nws = max(prev_global_end + ZC_PAIR_EPS, nws)
            min_dur = min(
                ZC_MIN_REMAINING_WORD_SEC,
                max(1e-3, (we0 - ws0) * 0.5),
            )
            if nwe <= nws + min_dur:
                nwe = min(dur_max, nws + max(min_dur, we0 - ws0))
            nwe = max(nwe, nws + 1e-4)
            refined.append({**w, "start": nws, "end": nwe})
        ...
        out.append(
            {
                **{k: v for k, v in cue.items() if k != "words"},
                "start": min(float(x["start"]) for x in refined),
                "end": max(float(x["end"]) for x in refined),
                "text": _rebuild_cue_text(refined),
                "words": refined,
            }
        )
    return out
...
def apply_rms_vad_word_align(
    ...
    *,
    ...
    if zero_cross_refine and word_snap:
        out_cues = _apply_zero_cross_refine_to_cues(out_cues, samples, dur_max)
