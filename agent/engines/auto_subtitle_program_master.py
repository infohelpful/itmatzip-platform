...
from engines.auto_subtitle_bake_segments import (
    ...
    validate_bake_segments_duration,
)
...
def _run_program_master_filter_encode(
    preview_media: Path,
    out_path: Path,
    segments: list[tuple[float, float]],
    probe: dict[str, Any],
    *,
    ...
    timeout_sec: float,
    audio_crossfade_sec: float | None = BAKE_PROGRAM_MASTER_AUDIO_FADE_SEC,
...
def bake_program_master(
    ...
    *,
    ...
    literal_segments = program_clips_to_literal_bake_segments(clips)
    segments, program_slots, prep_meta = prepare_bake_segments(
        preview_media,
        literal_segments,
    ...
    validate_bake_segments_duration(
        ...
        expected_from_clips=float(clips[-1].get("programEnd") or 0) if clips else 0,
        raw_clip_count=len(clips),
        program_slot_durations=program_slots,
    )
    if not segments:
    ...
    bake_level = "filter"
    encoder = "copy"
    bake_extra: dict[str, Any] = {
        "bake_prepare": prep_meta,
        "preview_parity": True,
        "program_slot_count": len(program_slots or []),
    }
        ...
        if l1_ok and out_path.is_file():
            ...
            if _duration_gate_ok(expected, actual):
                metrics = _finalize_bake_metrics(
                    ...
                    probe_out=probe_out,
                    encoder=encoder,
                    extra={**bake_extra, "l1": l1_metrics},
                )
                _log.info(
    ...
    _run_program_master_filter_encode(
        preview_media,
        out_path,
        segments,
        probe,
        ...
        on_progress=on_progress,
        timeout_sec=timeout_sec,
        audio_crossfade_sec=BAKE_PROGRAM_MASTER_AUDIO_FADE_SEC,
        program_slot_durations=program_slots,
    )
    encoder = select_burn_in_h264_encoder(str(get_ffmpeg_executable()))
    ...
    metrics = _finalize_bake_metrics(
        ...
        probe_out=probe_out,
        encoder=encoder,
        extra={**bake_extra, "audio_crossfade_sec": BAKE_PROGRAM_MASTER_AUDIO_FADE_SEC},
    )
