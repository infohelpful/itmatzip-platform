"""IOPaint LaMa (erase-only) — MSI engine Python 3.12 + engine-runtime/magic-eraser (library-hub wheels).

전체 iopaint 패키지(Stable Diffusion 스택 포함)는 무겁기 때문에 설치하지 않는다.
torch/torchvision + numpy/Pillow/opencv 만 wheel 로 오프라인 설치하고,
LaMa TorchScript 가중치(big-lama.pt)만 내려받아 `iopaint_runner.py` 에서 직접 추론한다.
"""
...
MAGIC_ERASER_LIB_BASE = (
    "https://github.com/infohelpful/library-hub/releases/download/magic-eraser-lib"
)

DEFAULT_WHEELS_CPU_URL = f"{MAGIC_ERASER_LIB_BASE}/iopaint-wheels.zip"
DEFAULT_WHEELS_GPU_PART_URLS = (
    f"{MAGIC_ERASER_LIB_BASE}/iopaint-wheels_gpu.zip.001",
    f"{MAGIC_ERASER_LIB_BASE}/iopaint-wheels_gpu.zip.002",
)
DEFAULT_LAMA_MODEL_URL = f"{MAGIC_ERASER_LIB_BASE}/big-lama.pt"

# hub 자산이 같은 파일명으로 교체될 때 캐시 무효화
IOPAINT_WHEELS_BUNDLE_REVISION = "cp312-lama-v1"

# erase-only — diffusers/transformers 등 SD 스택은 제외
IOPAINT_PIP_PACKAGES = (
    "numpy",
    "Pillow",
...
# opencv-python-headless -> cv2, Pillow -> PIL (설치되는 실제 디렉터리 이름 기준)
_RUNTIME_MARKER_DIRS = ("torch", "numpy", "cv2", "PIL")
...
def magic_eraser_root() -> Path:
    return Path(os.environ.get("APPDATA", Path.home() / ".itmatzip")) / "ItMatZip" / "magic-eraser"


def iopaint_site_packages() -> Path:
    """pip --target / 추론 site-packages (engine-runtime)."""
    return runtime_site_packages_dir(TOOL_MAGIC_ERASER)


def iopaint_python() -> Path:
    """추론·probe용 실행 파일 — MSI/엔진 python (3.12)."""
    explicit = os.environ.get("ITMATZIP_MAGIC_ERASER_PYTHON", "").strip()
    if explicit:
...
def models_root() -> Path:
    path = magic_eraser_root() / "models"
    path.mkdir(parents=True, exist_ok=True)
    return path


def lama_model_path() -> Path:
    return models_root() / "big-lama.pt"


def wheels_cache_dir() -> Path:
    path = magic_eraser_root() / "wheels-cache"
    path.mkdir(parents=True, exist_ok=True)
    return path.resolve()
...
def _config_path() -> Path:
    return magic_eraser_root() / "config.json"
...
def _wheels_cpu_url() -> str:
    raw = os.environ.get("ITMATZIP_MAGIC_ERASER_WHEELS_CPU_URL", "").strip()
    if raw:
        return raw
    cfg = str(_load_config().get("magic_eraser_wheels_cpu_url") or "").strip()
    return cfg or DEFAULT_WHEELS_CPU_URL


def _wheels_gpu_part_urls() -> tuple[str, str]:
    part1 = os.environ.get("ITMATZIP_MAGIC_ERASER_WHEELS_GPU_PART1_URL", "").strip()
    part2 = os.environ.get("ITMATZIP_MAGIC_ERASER_WHEELS_GPU_PART2_URL", "").strip()
    if part1 and part2:
        return part1, part2
    cfg = _load_config()
    cfg1 = str(cfg.get("magic_eraser_wheels_gpu_part1_url") or "").strip()
    cfg2 = str(cfg.get("magic_eraser_wheels_gpu_part2_url") or "").strip()
    if cfg1 and cfg2:
        return cfg1, cfg2
    return DEFAULT_WHEELS_GPU_PART_URLS


def lama_model_url() -> str:
    raw = os.environ.get("ITMATZIP_MAGIC_ERASER_LAMA_URL", "").strip()
    if raw:
        return raw
    cfg = str(_load_config().get("magic_eraser_lama_url") or "").strip()
    return cfg or DEFAULT_LAMA_MODEL_URL
...
def download_http_file(
    ...
    *,
    ...
    headers = {
        "User-Agent": "ItMatZip-Agent-MagicEraser/1.0",
        "Accept": "application/octet-stream,*/*",
    }
    token = (
        os.environ.get("ITMATZIP_MAGIC_ERASER_LIB_TOKEN", "").strip()
...
def _wheel_bundle_marker_payload(bundle: str) -> str:
    ...
    return f"{IOPAINT_WHEELS_BUNDLE_REVISION}\n{body}"
...
def _wheel_bundle_cache_valid(bundle: str) -> bool:
    ...
    if _normalize_marker_text(marker.read_text(encoding="utf-8")) != _normalize_marker_text(
        ...
        return False
    for probe in ("torch", "numpy", "opencv-python-headless"):
...
def ensure_wheels_bundle_extracted(
    ...
    *,
    ...
    if bundle == "cpu":
        zip_path = cache / "iopaint-wheels.zip"
        download_http_file(_wheels_cpu_url(), zip_path, message_cb=msg, label="IOPaint CPU wheel")
        ...
        part_paths = [parts_dir / f"iopaint-wheels_gpu.zip.{i:03d}" for i in (1, 2)]
        for idx, (url, part_path) in enumerate(zip(urls, part_paths, strict=True), start=1):
            download_http_file(
                url,
                part_path,
                message_cb=msg,
                label=f"IOPaint GPU wheel ({idx}/2)",
            )
        msg("wheel zip 병합 중…")
        merged = cache / "iopaint-wheels_gpu.zip"
...
def _extract_wheel_into_site(wheel_path: Path, site: Path) -> None:
    """pip 없이 .whl → site-packages 직접 해제 (Vocal Remover·Image Enhancer·BiRefNet과 동일)."""
    ...
    *,
    ...
    if missing:
        raise RuntimeError(
            ...
            + ", ".join(missing)
            + f". library-hub magic-eraser-lib 번들을 갱신하세요. (dir={wheel_dir})"
        )
    finalize_runtime_pip(TOOL_MAGIC_ERASER)


def _purge_torch_stack() -> None:
    purge_runtime_site_entries(
        TOOL_MAGIC_ERASER, "torch", "torchvision", "torchaudio", "functorch"
    )
    finalize_runtime_pip(TOOL_MAGIC_ERASER)
...
def install_pip_packages_from_wheels(
    ...
    required = [_package_name_from_spec(p) for p in IOPAINT_PIP_PACKAGES]
    missing = [
    ...
    if missing:
        raise RuntimeError(
            ...
            + ". library-hub magic-eraser-lib 의 iopaint-wheels 를 완전 세트로 교체하세요."
        )
...
def install_runtime_dependencies(
    ...
    *,
    ...
    if is_pip_stack_ready_fast():
        _emit(on_progress, 45.0, "pip 패키지", "numpy · Pillow · opencv 이미 설치됨")
    else:
...
def runtime_env() -> dict[str, str]:
    """추론 subprocess 공통 env — engine-runtime site-packages + UTF-8."""
    env = os.environ.copy()
    env["PYTHONNOUSERSITE"] = "1"
    # 파이프로 연결된 stdout 은 기본이 로케일 인코딩(cp949)이라 한글 진행 메시지가 깨진다.
    env["PYTHONIOENCODING"] = "utf-8"
    env["PYTHONUTF8"] = "1"
    env["ITMATZIP_RUNTIME_TOOL"] = TOOL_MAGIC_ERASER
    return env


def _run_iopaint_python(
    args: list[str],
    *,
    cwd: Path | None = None,
    timeout: float = 900.0,
...
def select_torch_bundle() -> str:
    variant = os.environ.get("ITMATZIP_MAGIC_ERASER_TORCH_VARIANT", "auto").strip().lower()
    if variant == "cpu":
        return "cpu"
    if variant == "gpu":
        return "gpu"
...
def probe_torch(timeout: float = 90.0) -> dict[str, object]:
    try:
        proc = _run_iopaint_python(["-c", _TORCH_PROBE_SCRIPT], timeout=timeout)
...
def iopaint_importable(module_name: str) -> bool:
    proc = _run_iopaint_python(["-c", f"import {module_name}"], timeout=180)
...
def is_pip_stack_ready() -> bool:
    if not is_pip_stack_ready_fast():
        return False
    try:
        return all(iopaint_importable(name) for name in ("numpy", "PIL", "cv2"))
    except (OSError, subprocess.TimeoutExpired, RuntimeError) as exc:
...
def is_lama_model_ready() -> bool:
    path = lama_model_path()
    return path.is_file() and path.stat().st_size > _MIN_LAMA_MODEL_BYTES
...
def ensure_lama_model(
    ...
    *,
    ...
    """library-hub 에서 LaMa TorchScript 가중치(big-lama.pt)를 내려받는다."""
    path = lama_model_path()
    if is_lama_model_ready():
        _emit(on_progress, base_pct + 30.0, "AI 모델", "big-lama.pt 이미 설치됨")
        return path

    url = lama_model_url()
    _emit(on_progress, base_pct, "AI 모델", "LaMa(big-lama.pt) 다운로드")
    download_http_file(
        url,
        path,
        message_cb=lambda text: _emit(on_progress, base_pct + 5.0, "AI 모델", text),
        label="LaMa big-lama.pt",
    )
    if not is_lama_model_ready():
        raise RuntimeError(f"LaMa 모델 설치를 확인할 수 없습니다: {path}")
    return path


def ensure_runtime(on_progress: PrepareProgressCallback | None = None) -> None:
    """engine-runtime/magic-eraser 준비."""
    magic_eraser_root().mkdir(parents=True, exist_ok=True)
    site = iopaint_site_packages()
    ensure_runtime_tree_acl(TOOL_MAGIC_ERASER)
    _emit(on_progress, 10.0, "런타임", f"engine-runtime · {site}")
    if is_runtime_ready_fast():
        _emit(on_progress, 12.0, "런타임", "이미 준비됨")
        return
    _emit(on_progress, 11.0, "런타임", str(iopaint_python()))
...
def runtime_status_fast() -> dict[str, Any]:
    ...
    return {
        "site_packages": str(iopaint_site_packages()),
        "python": str(iopaint_python()),
        "runtime": "magic-eraser-engine-runtime",
        "runtime_ready": ready_fast,
        "torch_ready": ready_fast,
        "pip_stack_ready": is_pip_stack_ready_fast(),
        "model_ready": is_lama_model_ready(),
        "msi_python_bundle": True,
        "library_hub_base": MAGIC_ERASER_LIB_BASE,
        ...
        "bundle_revision": IOPAINT_WHEELS_BUNDLE_REVISION,
    }
