/**
 * AutoSubtitle SubtitleVirtualList.tsx ??Í∞Ä???§ÌÅ¨Î°§¬∑Ï§Ñ Ï§?Ïπ©¬∑Ï∫ê?ø¬∑ÌÇ§Î≥¥Îìú.
 */
...
/** @type {number} is-playing ?¥Îûò?§Í? Î∂ôÏ? Ïπ¥Îìú */
let lastIsPlayingCardIdx = -1;
/** @type {number} is-active (?¨ÏÉù Ï§? */
let lastActivePlayingIdx = -1;
/** @type {number} is-active (?†ÌÉù Ï§? */
...
function renderAllCards(container, cues, opts) {
  ...
  for (const i of indices) {
    ...
    rail.setAttribute("aria-label", "?®Ïñ¥ ?Ä?ÑÎùº??);
    ...
    ta.setAttribute("aria-label", "?êÎßâ ?∏Ïßë");
...
export function updatePlaybackHighlights(container, cues, opts) {
  ...
  /* ?¨ÏÉù Ï§??êÎèô ?§ÌÅ¨Î°??ÜÏùå ??scrollCueIntoView ??scrollActiveCard ???åÎßå script Ï™ΩÏóê??*/
}
