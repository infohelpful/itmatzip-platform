...
const TOOL_API = "/api/tools/watermark-remover";

const STORAGE_OUTPUT = "watermark-remover:dl-output-path";
const STORAGE_SOURCE = "watermark-remover:dl-source-name";
const STORAGE_RETURN_FROM_DL = "watermark-remover:return-from-dl";
...
function startCountdown(outputPath) {
  ...
  setStatus(`${AUTO_START_SEC}초 후 결과 영상을 자동 다운로드합니다…`);
  ...
  countdownTimer = window.setInterval(() => {
    ...
    clearCountdown();
    void runDownload(outputPath, "결과 영상");
  }, 1000);
}

async function initPage() {
  document.title = "Watermark Remover · 다운로드";
  const session = readSession();
  if (!session.outputPath) {
    setStatus("다운로드할 결과가 없습니다. 편집 화면에서 먼저 워터마크 제거를 완료해 주세요.", "err");
    if (elMeta) elMeta.textContent = "결과 파일 경로가 저장되지 않았습니다.";
    return;
  }
  if (elMeta) {
    elMeta.textContent = session.source
      ? `원본: ${session.source} · 출력: 워터마크 제거 영상`
      : "출력: 워터마크 제거 영상";
  }
